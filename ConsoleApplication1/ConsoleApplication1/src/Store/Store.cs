...
//     * @exception  ItemNotFound  */
    public  Collection  searchItems(String keyWord, String itemName, String category) {

        Collection result = new HashSet();
        result.addAll(inventory.getItems().keySet());
        bool itemValue = itemName != null && !itemName.trim().isEmpty();
        bool categoryValue = category != null && !category.trim().isEmpty();
        bool keyWordValue = keyWord != null && !keyWord.trim().isEmpty();
        if(!itemValue && !categoryValue && !keyWordValue)
            return result;
        if (itemValue)
        {
            Collection am;
            am = inventory.searchItemByName(itemName);
            result.retainAll(am);
        }
    ...
    public Collection filterItems(Collection product, double ratingItem, double ratingStore,
                                                   double maxPrice, double minPrice) {
        Collection result = new LinkedList();
        if(ratingItem != null)
            result.retainAll(inventory.filterByRating( ratingItem));
        if(ratingStore != null && rating < ratingStore)
            result = new HashSet();
        if(minPrice != null && maxPrice != null)
            result.retainAll(inventory.filterByPrice(minPrice, maxPrice));
        return result;
...
//     *  @param startPrice - the startPrice of the items price
//     * @param endPrice - the endPrice of the items price */
    public Collection filterByPrice(Collection products, double startPrice, double endPrice) {
        if (products != null)
            return this.inventory.filterByPrice(startPrice, endPrice);
        return this.inventory.filterByPrice(startPrice, endPrice);
    }
...
//     * @exception ItemNotFoundException - On non existing item with param rating or greater*/
    public Collection filterByRating(Collection products, double rating) {
        if (products != null)
...
//    }
    ...
    public void rollBack(ConcurrentHashMap products) {
        ...
        {
            int update = (int)inventory.getItems().get(entry.Key);
            update += entry.Value;
            inventory.getItems().put(entry.Key, update);
        }
